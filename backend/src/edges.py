...
MINDMAP_PROMPT = (
    ...
    "       - 'sourceHandlePosition' (string, optional): EXACT position for the node's SOURCE handle (output). MUST be one of: 'top', 'bottom', 'left', 'right'.\n"
    "       - 'targetHandlePosition' (string, optional): EXACT position for the node's TARGET handle (input). MUST be one of: 'top', 'bottom', 'left', 'right'.\n"
    "         CRITICAL:\n"
    "           * sourceHandlePosition and targetHandlePosition MUST be DIFFERENT (never use the same side for both).\n"
    "           * Choose them based on geometry between parent and child nodes:\n"
    "               - If child is mostly to the RIGHT of parent: parent.sourceHandlePosition='right', child.targetHandlePosition='left'\n"
    "               - If child is mostly to the LEFT of parent: parent.sourceHandlePosition='left', child.targetHandlePosition='right'\n"
    "               - If child is mostly BELOW parent: parent.sourceHandlePosition='bottom', child.targetHandlePosition='top'\n"
    "               - If child is mostly ABOVE parent: parent.sourceHandlePosition='top', child.targetHandlePosition='bottom'\n"
    "               - For diagonal positions, use the dominant axis: if |dx| > |dy|, use left/right; otherwise use top/bottom.\n"
    "           * This ensures edges leave from the side facing the child and enter from the side facing the parent.\n"
    ...
    "     * HANDLE CONFIGURATION (MANDATORY):\n"
    "       - Each node MUST effectively have exactly ONE source handle and ONE target handle on the frontend.\n"
    "       - You control this via node.data.sourceHandlePosition and node.data.targetHandlePosition.\n"
    "       - Both MUST be one of: 'top', 'bottom', 'left', 'right'.\n"
    "       - They MUST be DIFFERENT (never use the same side for both).\n"
    "       - Choose them based on geometry:\n"
    "           * If child is mostly to the RIGHT of parent: parent.sourceHandlePosition='right', child.targetHandlePosition='left'.\n"
    "           * If child is mostly to the LEFT of parent: parent.sourceHandlePosition='left', child.targetHandlePosition='right'.\n"
    "           * If child is mostly BELOW parent: parent.sourceHandlePosition='bottom', child.targetHandlePosition='top'.\n"
    "           * If child is mostly ABOVE parent: parent.sourceHandlePosition='top', child.targetHandlePosition='bottom'.\n"
    "           * For diagonal positions, use the dominant axis: if |dx| > |dy| use left/right; otherwise use top/bottom.\n"
    "       - This keeps edges straight and attached to the side that visually faces the connected node.\n"
